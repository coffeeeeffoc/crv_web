...
const DecimalEdit: FC<DataEditProps> = (props) => {
                ...
                row.map((field: any, colIndex: number) => {
                  ...
                  if (field.name === 'decimalDigits') {
                    ...
                    newItemProps.rules = [
                      ...
                      { min: 2, max: currencyLength > 30 ? 30 : currencyLength, type: 'integer', message: `小数位数应为整数且应大于等于2,小于等于${currencyLength > 30 ? 30 : '字段长度'}` }
                    ]
                  }
                  // 小数位数的依赖关系
                  if (field.name === 'showDigits') {
                    ...
                    newItemProps.rules = [
                      { max: decimalDigits, type: 'integer', message: `显示小数位数应为整数且小于等于${decimalDigits < 30 ? '字段小数位数' : 30}` }
                    ]
                  }
                  ...
                  return (
                    <Col span={span} key={`lastCol-${field.name}`}>
