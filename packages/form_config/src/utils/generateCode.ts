...
function declaration (name: any, value: any, def: any) {
  return value !== def ? `\n  ${name}: ${value};` : '';
}

export function areaToCSS (area: AreaState, { parentGrid, templateAreas = true, repeat, oldSpec }: any) {
  const { id, grid } = area;
  const singleLine = areaIsSingleLineInCSS(area);
  const cssName = toCssName(id);
...
export function gridToCSS (area: AreaState, { templateAreas = true, repeat }: any) {
  const { grid } = area;
  if (!grid) {
    return '';
  }
...
export function identString (ident: number) {
  return '  '.repeat(ident);
};

function areasToHTML (area: AreaState, ident = 0) {
  let html = '';
...
export function areaToHTML (area: AreaState, ident = 0) {
  const tag = getElementTag(area);
  return `<${tag} ${includeAreaInCSS(area) ? `class="${toCssName(area.id)}"` : ''}>${areasToHTML(
    area,
    ident + 1
  )}</${tag}>`;
};

export function presentationCSS (area: AreaState) {
  const cssName = toCssName(area.id);
